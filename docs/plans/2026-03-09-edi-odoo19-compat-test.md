...
**Tech Stack:** Python + paramiko (SSH/SFTP), PostgreSQL 15, Odoo 19 (`/opt/odoo19/odoo-bin`), server at `10.0.0.35` (credentials in env vars).
...
### SSH notes
- `jono` has full sudo (`(ALL : ALL) ALL`) with password `***REDACTED***`. Use `echo '***REDACTED***' | sudo -S <cmd>` for any privileged command.
...
def ssh():
    ...
    c.connect('10.0.0.35', username='jono', password='***REDACTED***', timeout=15)
...
```python
client = ssh()
out, _ = run(client, "echo '***REDACTED***' | sudo -S -u postgres psql -l 2>&1")
print(out)
client.close()
```
...
```python
client = ssh()
out, _ = run(client, "echo '***REDACTED***' | sudo -S systemctl is-active odoo19 2>&1")
print(out)
client.close()
```
...
```python
client = ssh()
out, _ = run(client, "echo '***REDACTED***' | sudo -S -u postgres psql -c \"SELECT count(*) FROM pg_stat_activity WHERE datname='MML_Test_Odoo19_02102026';\" 2>&1")
print(out)
client.close()
```
...
```python
client = ssh()
out, err = run(client, "echo '***REDACTED***' | sudo -S -u postgres createdb -O odoo -E UTF8 MML_EDI_Compat 2>&1")
print(out, err)
client.close()
```
...
out, err = run(client,
    "echo '***REDACTED***' | sudo -S -u postgres pg_dump MML_Test_Odoo19_02102026 -Fc -f /tmp/mml_compat_dump.pgc 2>&1",
    timeout=600
)
...
client = ssh()
out, err = run(client,
    "echo '***REDACTED***' | sudo -S -u postgres pg_restore -d MML_EDI_Compat /tmp/mml_compat_dump.pgc 2>&1",
    timeout=600
)
...
client = ssh()
out, _ = run(client,
    "echo '***REDACTED***' | sudo -S -u postgres psql -d MML_EDI_Compat -c \"SELECT count(*) FROM ir_module_module WHERE state='installed';\" 2>&1"
)
print(out)
client.close()
```
...
```python
client = ssh()
run(client, "echo '***REDACTED***' | sudo -S chmod -R a+rX /tmp/mml_compat/")
client.close()
```
...
```python
client = ssh()
out, err = run(client, "echo '***REDACTED***' | sudo -S systemctl stop odoo19 2>&1")
print(out, err)
# Verify it stopped
out2, _ = run(client, "echo '***REDACTED***' | sudo -S systemctl is-active odoo19 2>&1")
print("Status:", out2)
client.close()
```
...
client = ssh()
out, _ = run(client,
    "echo '***REDACTED***' | sudo -S -u postgres psql -d MML_EDI_Compat "
...
for q in queries:
    o, _ = run(client, f"echo '***REDACTED***' | sudo -S -u postgres psql -d MML_EDI_Compat -c \"{q}\" 2>&1")
    print(q)
    print(o)
client.close()
```
...
client = ssh()
out, _ = run(client,
    "echo '***REDACTED***' | sudo -S tail -100 /var/log/odoo19/odoo.log 2>&1 | grep -E 'ERROR|CRITICAL'"
)
...
run(client,
    "echo '***REDACTED***' | sudo -S -u postgres psql "
    "-c \"SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname='MML_EDI_Compat';\" 2>&1"
)
out, err = run(client,
    "echo '***REDACTED***' | sudo -S -u postgres dropdb MML_EDI_Compat 2>&1"
)
print(out, err)
client.close()
```
...
```python
client = ssh()
out, err = run(client, "echo '***REDACTED***' | sudo -S rm -rf /tmp/mml_compat/ 2>&1")
print(out, err)
client.close()
```
...
```python
client = ssh()
out, err = run(client, "echo '***REDACTED***' | sudo -S systemctl start odoo19 2>&1")
...
out, _ = run(client,
    "echo '***REDACTED***' | sudo -S -u postgres psql -l 2>&1"
)
...
client = ssh()
out, _ = run(client,
    "echo '***REDACTED***' | sudo -S tail -30 /var/log/odoo19/odoo.log 2>&1 | grep -E 'ERROR|CRITICAL|odoo.service'"
)
...
if rc != 0:
    rc2, out2, err2 = run(client,
        "echo '***REDACTED***' | sudo -S /opt/odoo19/venv/bin/pip install cryptography 2>&1",
        timeout=120
    )
...
```python
client = connect()
run(client, "echo '***REDACTED***' | sudo -S systemctl stop odoo19.service 2>&1")

cmd = (
    "echo '***REDACTED***' | sudo -S -u odoo "
...
```python
client = connect()
run(client, "echo '***REDACTED***' | sudo -S systemctl start odoo19.service 2>&1")
time.sleep(5)
rc, out, _ = run(client, "echo '***REDACTED***' | sudo -S systemctl is-active odoo19.service 2>&1")
print("service:", out)
...
for q in checks:
    rc, out, _ = run(client,
        f"echo '***REDACTED***' | sudo -S -u postgres psql -d MML_EDI_Compat -c \"{q}\" 2>&1"
    )
